...
combineParams dict ( route, params ) =
    ( route, Dict.union params dict )
...
unwrap raw =
    let
        ...
        matches =
            Regex.find (Regex.AtMost 1) regex raw
                ...
                [ match ] ->
                    case match.submatches of
                        [ Just a, Just b, Just c ] ->
                            List.concat <| List.map unwrap [ a ++ b ++ c, a ++ c ]
    ...
    let
        ...
        strings =
            case params of
                ...
                p ->
                    Regex.split Regex.All (Regex.regex <| String.join "|" <| List.map (String.cons paramChar) p) raw

        sringParsers =
            List.map Combine.string strings

        paramParsers =
            List.map
                (\param ->
                    Maybe.withDefault stringParser <|
                        Maybe.map
                            (\c ->
                                case c of
                                    Int ->
                                        toString <$> Combine.Num.int

                                    Enum list ->
                                        Combine.choice <| List.map Combine.string list
                        ...
                        <|
                            Dict.get param constraints
                )
                params

        last =
            case List.Extra.last sringParsers of
                Nothing ->
                    Debug.crash "List.Extra.last sringParsers"

                Just v ->
                    v

        parsers =
            List.map2 (\p1 p2 -> p1 *> p2) sringParsers paramParsers

        parser =
            (List.foldr (\p pacc -> p |> Combine.andThen (\r -> (++) r <$> pacc))
                (singleton <$> last)
        ...
        -- <* Combine.end
        ( result, context ) =
            Combine.parse parser url

        zipValues values =
            Dict.fromList <| List.map2 (,) params values
    in
        ( Result.map zipValues result, context.input )


matchInternal : (String -> List String) -> (route -> RouteConfig route state) -> List route -> List route -> URL -> Maybe (Route route)
matchInternal unwrap getConfig routes pool url =
    List.foldl
        (\route match ->
            case match of
                Just _ ->
                            ...
                            (\raw match_ ->
                                case match_ of
                                    ...
                                    Nothing ->
                                        let
                                            ( result, url_ ) =
                                                parseUrlParams raw config.constraints url
                                        in
                                            case result of
                                                Err _ ->
                                                    Nothing
                                                ...
                                                Ok dict ->
                                                    let
                                                        matchChildren route =
                                                            let
                                                                ( childrens, pool_ ) =
                                                                    filterParent (.parent << getConfig) (Just route) pool

                                                                child =
                                                                    matchInternal unwrap getConfig childrens pool_ url_

                                                                childRoute =
                                                                    Maybe.map (combineParams dict) child
                                                            in
                                                                childRoute
                                                    in
                                                        case config.bypass of
                                                            True ->
                                                                matchChildren route

                                                            False ->
                                                                case String.isEmpty url_ of
                                                                    True ->
                                                                        Just ( route, dict )
...
match_ : (String -> List String) -> (route -> RouteConfig route state) -> List route -> URL -> Maybe (Route route)
match_ unwrap getConfig routes url =
    let
        ( roots, pool ) =
            filterParent (.parent << getConfig) Nothing routes
    in
        matchInternal unwrap getConfig roots pool url


match : (route -> RouteConfig route state) -> List route -> URL -> Maybe (Route route)
match getConfig routes url =
    match_ unwrap getConfig routes url
...
buildRawUrl raws ( route, params ) =
    let
        ...
        urls_ =
            List.filter (not << String.contains (String.fromChar paramChar)) urls
    in
        case List.head urls_ of
            Nothing ->
                Debug.crash <| "not enough params to build URL: " ++ toString route
...
composeRawUrl getSegment getParent route =
    let
        segments =
            List.map getSegment <| getPath getParent route
    in
...
buildUrl getSegment getParent ( route, params ) =
    let
        raws =
            unwrap <| composeRawUrl getSegment getParent route
    in
        buildRawUrl raws ( route, params )


getPathInternal : (route -> Maybe route) -> route -> List route -> List route
getPathInternal getParent route acc =
    case getParent route of
        Nothing ->
            route :: acc

        Just parent ->
            route :: getPathInternal getParent parent acc
...
getPath getParent route =
    List.reverse <| getPathInternal getParent route []
...
mapParams matcher routes params =
    flip List.map routes <|
        \route ->
...
routeDiff matcher from to =
    let
        ...
        fromRoute =
            Maybe.map Tuple.first from

        fromParams =
            Maybe.withDefault Dict.empty <| Maybe.map Tuple.second from
        ...
        fromPath =
            Maybe.withDefault [] <| Maybe.map matcher.traverse fromRoute

        toPath =
            matcher.traverse toRoute
        ...
        commons =
            List.length <|
                List.Extra.takeWhile (uncurry (==)) <|
                    List.map2 (,) fromPath_ toPath_
    in
        List.drop commons toPath
...
matcher (RouterConfig config) =
    let
        ...
        getSegment =
            .segment << config.routeConfig

        getParent =
            .parent << config.routeConfig

        segments =
            List.map getSegment routes

        urls =
            List.map composeUrl_ routes

        sids =
            List.map toString routes

        dict =
            Dict.fromList <| List.map2 (,) sids routes
        ...
        stringToRoute sid =
            case Dict.get sid dict of
                ...
                Nothing ->
                    Debug.crash <| "stringToRoute: " ++ sid

        composeUrl =
            memoFallback (\sid -> composeRawUrl getSegment getParent (stringToRoute sid)) sids

        getConfig =
            memoFallback (\sid -> config.routeConfig (stringToRoute sid)) sids

        traverse =
            memoFallback (\sid -> getPath getParent (stringToRoute sid)) sids

        unwrap_ =
            memoFallback unwrap (urls ++ segments)

        routeParams =
            memoFallback (getParams << getSegment << stringToRoute) sids
        ...
        buildUrl_ route =
            let
                raws =
                    unwrap_ <| composeUrl_ (Tuple.first route)
            in
                buildRawUrl raws route
    in
        { getConfig = getConfig_
        , buildUrl = buildUrl_
        , match = match_ unwrap_ getConfig_ routes
        , traverse = traverse_
        , routeParams = routeParams_
