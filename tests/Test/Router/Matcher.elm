...
testReversible =
    suite "reversible"
        [ test "match" <|
            assertEqual (Just "/") <|
                Maybe.map (buildUrl (.segment << config) (.parent << config)) <|
                    (match config routes "/")
        , test "fail by constraint" <|
            assertEqual Nothing <|
                Maybe.map (buildUrl (.segment << config) (.parent << config)) <|
                    (match config routes "/param")
        , test "equal with constraint" <|
            assertEqual (Just "/A") <|
                Maybe.map (buildUrl (.segment << config) (.parent << config)) <|
                    (match config routes "/A")
        , test "match 404" <|
...
testMapParams =
    let
        params =
            Dict.fromList [ ( "category", "param" ), ( "subcategory", "param2" ), ( "item", "4" ) ]
    in
        suite "mapParams"
            [ test "mapParams" <|
...
testRouteDiff =
    let
        routeDiff_ =
            routeDiff Test.Mock.Data.matcher
    in
        suite "routeDiff"
            [ test "length" <|
                assertEqual 1 <|
                    List.length <|
                        routeDiff_ Nothing ( Home, Dict.empty )
            , test "length" <|
                assertEqual 3 <|
                    List.length <|
                        routeDiff_ Nothing ( Subpage, Dict.empty )
            , test "no transition - no handlers" <|
                assertEqual 0 <|
                    List.length <|
                        routeDiff_ (Just ( Home, Dict.empty )) ( Home, Dict.empty )
            , test "unmatched params has no effects" <|
                assertEqual 0 <|
                    List.length <|
                        routeDiff_ (Just ( Home, Dict.empty )) ( Home, Dict.fromList [ ( "param1", "value1" ) ] )
            , test "matched params does matter" <|
                assertEqual 1 <|
                    List.length <|
                        routeDiff_ (Just ( Page, Dict.fromList [ ( "category", "bar" ) ] )) ( Page, Dict.fromList [ ( "category", "foo" ) ] )
            , test "matched params does matter" <|
                assertEqual 2 <|
                    List.length <|
                        routeDiff_ (Just ( Subpage, Dict.fromList [ ( "category", "bar" ) ] )) ( Subpage, Dict.fromList [ ( "category", "foo" ) ] )
            ]
